...
export async function generateMetadata({
  ...
  const data = await getSeoHubData('physical-ai-world-models', locale);
...
export default async function PhysicalAiWorldModelsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <>
      <Script
        id={`seo-hub-jsonld-physical-ai-world-models-${locale}`}
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
      />
      <WorldModelsSeoHubPage hubKey="physical-ai-world-models" locale={locale} />
    </>
  );
}
