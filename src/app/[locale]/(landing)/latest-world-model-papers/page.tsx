...
export async function generateMetadata({
  ...
  const data = await getSeoHubData('latest-world-model-papers', locale);
...
export default async function LatestWorldModelPapersPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <>
      <Script
        id={`seo-hub-jsonld-latest-world-model-papers-${locale}`}
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
      />
      <WorldModelsSeoHubPage hubKey="latest-world-model-papers" locale={locale} />
    </>
  );
}
