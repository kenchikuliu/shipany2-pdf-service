...
export async function generateMetadata({
  ...
  const data = await getSeoHubData('world-model-projects', locale);
...
export default async function WorldModelProjectsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <>
      <Script
        id={`seo-hub-jsonld-world-model-projects-${locale}`}
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
      />
      <WorldModelsSeoHubPage hubKey="world-model-projects" locale={locale} />
    </>
  );
}
