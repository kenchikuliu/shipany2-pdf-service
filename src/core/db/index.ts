...
function loadDbModule<T = any>(modulePath: string): T {
...
export function db(): any {
  ...
  if (['sqlite', 'turso'].includes(envConfigs.database_provider)) {
    return withSqliteCompat(getSqliteModule().getSqliteDb() as any);
  }

  if (envConfigs.database_provider === 'mysql') {
    return withMysqlCompat(getMysqlModule().getMysqlDb() as any);
  }

  return getPostgresModule().getPostgresDb() as any;
}

export function dbPostgres(): any {
  if (envConfigs.database_provider !== 'postgresql') {
    throw new Error('Database provider is not PostgreSQL');
  }

  return getPostgresModule().getPostgresDb();
}

export function dbMysql(): any {
  if (envConfigs.database_provider !== 'mysql') {
    throw new Error('Database provider is not MySQL');
  }

  return getMysqlModule().getMysqlDb();
}

export function dbSqlite(): any {
  if (!['sqlite', 'turso'].includes(envConfigs.database_provider)) {
    throw new Error('Database provider is not SQLite');
  }

  return getSqliteModule().getSqliteDb();
}
...
export async function closeDb() {
  if (envConfigs.database_provider === 'postgresql') {
    await getPostgresModule().closePostgresDb();
    return;
  }

  if (envConfigs.database_provider === 'mysql') {
    await getMysqlModule().closeMysqlDb();
    return;
  }
}
