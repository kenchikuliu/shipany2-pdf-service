...
export const doc = table(
  ...
  (table) => [
    index('idx_doc_locale_status').on(table.locale, table.status),
    index('idx_doc_parent_sort').on(table.parentId, table.sort),
    index('idx_doc_path_locale').on(table.path, table.locale),
    index('idx_doc_level').on(table.level),
  ]
);

// Navigation Directory Tables
export const navCategory = table(
  'nav_category',
  {
    id: varchar191('id').primaryKey(),
    name: varchar191('name').notNull(),
  ...
  (table) => [
    index('idx_nav_category_status_sort').on(table.status, table.sort),
    index('idx_nav_category_slug').on(table.slug),
  ]
);

export const navItem = table(
  'nav_item',
  {
    id: varchar191('id').primaryKey(),
    userId: varchar191('user_id')
    ...
    pricingModel: varchar('pricing_model', { length: 50 }), // Free | Paid
    discountCode: varchar191('discount_code'),
    githubUrl: text('github_url'),
    status: varchar('status', { length: 50 })
      .notNull()
      .default('pending_review'), // live | pending_review | rejected
  ...
  (table) => [
    index('idx_nav_item_status').on(table.status),
    index('idx_nav_item_user_id').on(table.userId),
    index('idx_nav_item_is_featured').on(table.isFeatured),
    index('idx_nav_item_slug').on(table.slug),
  ]
);
