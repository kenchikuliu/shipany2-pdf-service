...
export async function addPost(data: NewPost) {
  ...
  const [result] = await db().insert(post).values(insertData).returning();
...
export async function getLocalPost({
  ...
  const localPostData = localPost.data as any;
  ...
  const frontmatter = localPostData;
...
export async function getLocalPage({
  ...
  const localPageData = localPage.data as any;
  ...
  const frontmatter = localPageData;
